...
/** Placeholder while questions load. */
export function QueueDetailSkeleton({ rows = 5 }: QueueDetailSkeletonProps) {
  return (
    <div className="animate-pulse overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="grid grid-cols-[minmax(0,2fr)_minmax(0,8rem)] gap-4 border-b border-slate-100 bg-slate-50 px-4 py-3">
      ...
      <div className="divide-y divide-slate-100">
        {Array.from({ length: rows }).map((_, idx) => (
          <div
            key={idx}
            className="grid grid-cols-[minmax(0,2fr)_minmax(0,8rem)] items-center gap-4 px-4 py-4"
          >
